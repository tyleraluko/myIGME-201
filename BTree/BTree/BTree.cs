...
namespace BTree
{
    public partial class BTreeForm : Form
    {
        ...
        private void Exercise1__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * inserts 30 random numbers 1-51
             */
            for (int i = 0; i < 30; i++)
            {
                int randomNumber = random.Next(1, 52); //generate random number between 1 and 51
                root = BTree.Insert(root, randomNumber);
            }
        ...
        private void Exercise2__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * primes tree w/ seven data points
             */
            for (int i = 0; i < 7; ++i)
            {
                int optimalValue = 1 + (i * 7);
                node = new BTree(optimalValue, root, false);
            }

            /*
             * Tyler Aluko
             * then inserts 30 random numbers 1-51
             */
            for (int i = 0; i < 30; ++i)
            {
        ...
        private void Exercise3__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * inserts 15 random uppercase strings
             */
            //list of words, petals for armor
            string[] words = { "SIMMER", "LEAVE", "CINNAMON", "CREEPIN", "DESIRE", "HORSE", "FRIEND", "OVER", "ROSES", "WHY", "PURE", "TAKEN", "SUGAR", "BLOOM", "CRYSTAL" };
            for (int i = 0; i < 15; ++i)
            {
                //generate random index to select word from list
                ...
                node = new BTree(randomWord, root);
            }
        ...
        private void Exercise4__Click(object sender, EventArgs e)
        {
            ...
            BTree node = null;
            BTree root = null;
            Random random = new Random(); //for 15 random strings - TA

            // Your code here
            /*
             * Tyler Aluko
             * priming tree w/ code from PrimedButton__Click (as clarified in the questions section)
             */
            node = new BTree("M", null);
            root = node;
        ...
        private void Exercise5__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * priming the tree using the code from PrimedButton__Click
             */
            node = new BTree("M", null);
            root = node;
        ...
        private void Exercise6__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * inserted 15 new Person objects corresponding to family
             */
            root = new BTree(new Person("Tyler", 20), null); //ME!!
            //other family members...
            node = new BTree(new Person("Lola", 18), root);
            node = new BTree(new Person("Mother", 54), root);
            node = new BTree(new Person("Aunt Rita", 65), root);
            node = new BTree(new Person("Aunt Stefanie", 35), root);
            node = new BTree(new Person("Father", 60), root);
            node = new BTree(new Person("Skittles", 11), root);
            node = new BTree(new Person("Cela", 4), root);
            node = new BTree(new Person("Runner", 5), root);
            node = new BTree(new Person("Bo", 5), root);
            node = new BTree(new Person("Jesse", 18), root);
            node = new BTree(new Person("Chris", 20), root);
            node = new BTree(new Person("Jack", 21), root);
            node = new BTree(new Person("Aayan", 19), root);
            node = new BTree(new Person("PJ", 20), root);
        ...
        private void Exercise7__Click(object sender, EventArgs e)
        {
            ...
            /*
             * Tyler Aluko
             * i think this is what it's asking me to do?
             */
            root = new BTree(new Person("Tyler", 20), null) { isData = false }; //ME!!
            //other family members...
            node = new BTree(new Person("Lola", 18), root) { isData = false };
            node = new BTree(new Person("Mother", 54), root);
            node = new BTree(new Person("Aunt Rita", 65), root);
            node = new BTree(new Person("Aunt Stefanie", 35), root);
            node = new BTree(new Person("Father", 60), root) { isData = false };
            node = new BTree(new Person("Skittles", 11), root);
            node = new BTree(new Person("Cela", 4), root);
            node = new BTree(new Person("Runner", 5), root);
            node = new BTree(new Person("Bo", 5), root);
            node = new BTree(new Person("Jesse", 18), root) { isData = false };
            node = new BTree(new Person("Chris", 20), root) { isData = false };
            node = new BTree(new Person("Jack", 21), root);
            node = new BTree(new Person("Aayan", 19), root);
            node = new BTree(new Person("PJ", 20), root);
