...
namespace BTree
{
    ...
    public class BTree
    {
        ...
        /*
         * Tyler Aluko
         * inserts new node w/ specified data into binary tree
         * 
         * parameters
         * -> root, root of tree where node inserted
         * -> newData, data to be stored in new node
         */
        public static BTree Insert(BTree root, object newData)
        {
            //creates new BTree node w/ provided data and initializes children to null
            BTree newNode = new BTree(newData, null);

            if (root == null) //if tree is empty/null
            {
                root = newNode; //new node becomes root
            }
            else
            {
                AddChildNode(newNode, root); //otherwise add child node
            }
